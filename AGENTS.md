...
- **App chrome follows the reference exactly**: the app nav has NO active-state highlight (all links `text-[#323338] hover:bg-[#F5F6F8] hover:text-[#0073EA]` — `aria-current` is kept for a11y but renders nothing), there is **no app footer**, and page containers put the padding OUTSIDE the `max-w-7xl` container (`p-6`/`p-4 md:p-8` wrapper → `mx-auto max-w-7xl` inner) so the container keeps the full 1280px.
...
- **SQLite file location quirk**: `DATABASE_URL=file:../db/custom.db` has been observed to resolve against the process CWD in some runs — the live file may land OUTSIDE the repo (session 3+4 both found it at `<parent>/db/custom.db`). If `db:seed` reports boards that `ls db/` says don't exist, check the parent directory. Also: **never delete the DB file while `next dev` is running** — the server keeps the deleted inode and every mutation fails with SQLite 1032 "readonly database"; restart the server after any DB file swap.
- Kanban column headers and priority badges use the same tint recipe (`${color}20` 8-digit hex = 12.5% alpha) — keep the kanban count badges and `priorityBadgeStyle` in sync if the recipe ever changes.
