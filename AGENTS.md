...
- **Domain vocabulary is closed.** Statuses, priorities, the six board colors, and board visibility (stored `private`/`public`, labeled Private/Shared — see `visibilityLabel`) are `const` arrays in `src/lib/domain.ts`. UI components and API validation both derive from them — never hardcode a status string in a component. The palette is the reference's monday-canonical set (statuses `#c4c4c4`/`#ffcb00`/`#00c875`/`#e2445c` with **white pill text on all four**; priorities `#787d80`/`#ffcb00`/`#fdab3d`/`#e2445c` rendered as tinted text badges via `priorityBadgeStyle` — 12.5% alpha bg + colored text; board colors `#0073ea #00c875 #ffcb00 #e2445c #a25ddb #00d9ff`).
