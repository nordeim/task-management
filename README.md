...
| 🔐 Email + password auth | scrypt-hashed passwords, opaque session tokens in httpOnly cookies (30-day TTL), login/signup/sign-out flows, and **auth rate limiting** (5 failed logins per email+IP per 15 min — successes reset, so transient typos never lock you out; 10 signup attempts per IP per 15 min; 429 + `Retry-After` with a friendly inline message). **Signup mirrors the redesigned platform surface** — Email / Password / Confirm Password (no Full name; the account renders under its email prefix via `deriveSignupName`) with a Back-to-sign-in link and an inline mismatch error |
...
live in [`docs/screenshots/`](docs/screenshots/) — login, signup, dashboard, boards, all
five board views, analytics, the three board-header modals, and the
...
| Unit tests | `bun run test` | Vitest, colocated suites + `tests/db-path.test.ts` (**185 tests**) — the pure domain seams (status↔completed coupling, timeline window math, kanban grouping, distribution bars, saved-indicator format, task filter/sort pipeline, column visibility, group summary, relative time, reference palette, priority badge recipe, visibility labels, view-trigger labels, kanban card border token, group color options, route paths, nav active-state matcher, dynamic calendar weeks, summary labels, team avatar palettes, 404 titlecase helper, recent-task time format, kanban avatar gradient/initials, group-header status dots, distinct owner names, people-column palette, team workload counts, recent-activity items, board modal stats, overdue date boundary, **signup display-name derivation**) **plus the vendored-primitive anatomy contracts** (Badge cva + Button base/variants/sizes + Switch track/thumb + Select trigger/item + **Input/Textarea/Label bases** locked to the reference's OLD-shadcn decompiled strings) **plus the database-path contract** **plus the rate-limit contract** |
| E2E | `bun run test:e2e` | Playwright (Chromium) — run after `bun run build`; the config boots the standalone artifact (`reuseExistingServer` reuses a live server). 27 specs: auth golden path (+ invalid credentials + `from_url` redirect + login rate limiting: 5×401 → 429 + Retry-After + message + the login/signup surface structure specs pinning the platform redesign, incl. the mobile spacer footer contract), board status-pill round-trip persisted across reload, **dialog-based mutation paths (New Task dialog create → row-trash delete, board create → Options edit → delete round-trip, group-collapse persistence — self-healing specs, idempotent across runs)**, the mobile navigation menu in a touch context (**the Tailwind v4 hover-variant regression**), the route surface (case-insensitivity, styled 404, back/forward), and **computed-style parity contracts** (`e2e/parity.spec.ts` — the v3→v4 shadow/blur values and the slate-400 login focus ring, pinned color-space-neutral against the reference's compilers; plus the **background-token contracts**: the white `--background` across dialog panels, outline buttons, and switch thumbs, and the `#A0A0A0` board-header sub-row separators) |
...
vendored-primitive anatomy, `src/lib/rate-limit.test.ts` for the auth
limiter's window semantics — implement the pure function (green), then
