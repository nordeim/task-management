...
margin-direction, v3→v4 shadow/blur renames on every ported string —
the platform login page compiles under the reference's v3 play CDN, so
its strings carry the same renames) · shadcn/ui (new-york, OLD-shadcn
...
- **E2E (Playwright) — delivered session 21**: `e2e/` specs cover login →
  ...
  round-trip; mismatched-passwords inline error — no account created).
...
- **E2E spec conventions** (session 31): mutation specs follow the
  ...
  request contexts do NOT carry the browser session cookie); role-name
  locators on task titles need `exact: true` (substring matching collides
  with the row's "Actions for <title>" trigger).
