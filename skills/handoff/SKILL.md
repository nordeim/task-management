...
7. **Suggest next skills.** Include a "Suggested Skills" section naming which skills the fresh agent should invoke next via the Skill tool (e.g. `research`, `plan-writing`, `spec-driven-development`). This was the sole unique value in the compact `skills/handoff` fork and is now absorbed here.
...
4. Fill in every section of the template below, including Suggested Skills. Omit a section only if it is genuinely empty (e.g. no blockers) — mark it `None`.
...
## 8. Suggested Skills
<Which skills the fresh agent should invoke next, and why. Name the Skill tool targets explicitly. Keep it short and actionable.
- `research` — to verify claims against primary sources before coding
- `plan-writing` — to break open work into sequenced tasks
- `spec-driven-development` — if requirements are still ambiguous>
