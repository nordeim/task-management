...
**File name note:** **Redirect stub** — canonical skill is now **`risen-christ_SKILL.md`** (Church of the Risen Christ, Toa Payoh, 2026-08-31). Do not edit this file independently; all future updates go to `risen-christ_SKILL.md`. This file is retained for lineage diff (St Joseph → St Mary) only.
...
| CSP | No console CSP violations | Verify `index.html` CSP: `img-src 'self' data: blob:` only (all images local — legacy wikimedia/pexels allowlist removed round-6), `frame-src https://www.google.com` for maps embed; no `unsafe-eval`; `SafeImage` fallback `/images/hero-church.jpg` covers any future external image |
...
| Pre-ship gate | `pnpm lint && pnpm typecheck && pnpm test && pnpm test:e2e && pnpm build` → 31/172 + 45 E2E green → `dist/index.html` + `dist/_headers` + `dist/images/` (8 files) → `pnpm preview` → manual smoke (see §11) · Built-artifact: `pnpm test:e2e:built` guards `./favicon.svg` rewrite (round-9 E2E-L1) |
