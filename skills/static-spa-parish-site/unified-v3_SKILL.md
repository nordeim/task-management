...
canonical_ref: Church of the Risen Christ — Toa Payoh (91 Toa Payoh Central, Singapore 319193 — reference implementation; first air-con 1971, Fr Pierre Abrial, Easter Sunday)
lineage: Blessed Stanley Rother Shrine (OKC) → St Joseph BT (620 Upper Bukit Timah, 1845–2017) → St Mary of the Angels (5 Bukit Batok East Ave 2, 1957–2026) → Risen Christ (91 Toa Payoh Central, 1969–2026)
project_state: "Lineage Master — 77 files / ~202 tests + ~51 E2E + ~51 built-artifact (reference: Risen Christ Toa Payoh; counts are machine-asserted by docs-contract, not prose — see §2/§11) — governs all 4 hops, singlefile HashRouter SPA, 26 colors + 2 shadows superset"
...
port_provenance: Lineage Master governing the Singapore port chain — reference implementation is https://www.risenchrist.org.sg/ (Church of the Risen Christ, 91 Toa Payoh Central, blessed 3 July 1971, first air-con); lineage preserved in Appendices D+F — see lineage above
---

# `singapore-parish-lineage` — Master Engineering Skill (v3.0.0)

> **How to use this document:** This is the single-source-of-truth for the Singapore Parish SPA Lineage. **Canonical reference implementation is Church of the Risen Christ (Toa Payoh, 91 Toa Payoh Central)** — the most mature hop (35/202 + 51 E2E + 51 built-artifact, `useScrollSpy` + `deepLinks`, `docs-contract` + `repo-hygiene`). Read §§ 1–4 for identity and constraints (with `{{Parish X}}` template notes), §5 for where to put code, §§ 9–11 before shipping any port, and §§ 15–20 as copy-pasteable contracts. Every version, hex, and path is verified against the canonical `package.json` / `src/index.css` / `tsconfig.json` / `src/data/*` — if it drifts, fix this file first. Counts like `~35/202` are machine-asserted by `docs-contract`, not prose — see §2/§11.

**Sources of truth:** `README.md` (visitor overview) → `AGENTS.md` (60-sec cheat sheet) → `CLAUDE.md` (deep workflow, 6-phase) → this file (complete distillate). If they conflict, trust executable config. Note: `src.orig/` is **forbidden in the git index for the entire lineage** (Rother → St Joseph → St Mary → Risen Christ); its ignore entries in eslint/vite config are active guards — not inert. Lineage history lives in Appendices D/F + git history, not on disk.

**File name note:** **Lineage Master v3.0.0** — this file (`unified-v3_SKILL.md` / `singapore-parish-lineage`) governs all hops. Parish files `risen-christ_SKILL.md`, `st-mary-of-angels_SKILL.md`, and `rothershrine-v2_SKILL.md` are now **frozen profiles** — do not edit independently; all future architectural updates go here, parish-specific deltas live only in `src/data/site.ts` + `src/data/content.ts` + `public/images/`.

**Migration note:** This file is the Lineage Master distillate — built on the `risen-christ` §§1–20 + Appendices A–F skeleton (the most mature, round-12), elevated from parish-specific to lineage-wide. The sectional skeleton is preserved verbatim; parish narrative in §1 is now the 4-hop lineage (one breath per parish) with Risen Christ as the worked example. Design tokens are the **26+2 superset** (see §4). See Appendix D (Rother→St Joseph→St Mary) and Appendix F (St Mary→Risen Christ) for full provenance.
...
**Lineage in one breath (4 hops — architecture immutable, only parish facts change):** Blessed Stanley Rother Shrine (OKC, Oklahoma/Guatemala martyr 1935–2023) → **St Joseph BT** (620 Upper Bukit Timah Road, S678116) — M.E.P. Kranji attap chapel 1846 → Palladian on six Doric columns 1853 → 1964 Fr Joachim Teng rebuild (cattle milk, food fair, blessed 30 Aug 1964 by Abp Olçomendy), 1 May St Joseph the Worker, last Catholic cemetery + Rosary Garden 2017 → **St Mary of the Angels** (5 Bukit Batok East Ave 2, S659918) — OFM friars 1957 → Portiuncula 1970 → WOHA folded-concrete-and-timber house of light consecrated 2004 (SIA award, 8,500 seats, Design of the Year 2006), 2 Aug Portiuncula → **Risen Christ** (91 Toa Payoh Central, S319193) — Ho Ping Centre 1969 → first air-con nave blessed 3 July 1971 by Abp Olçomendy (Fr Pierre Abrial, $450k) → Velankanni devotion + Simbang Gabi, Grateful Faithful Sent 2026. **Only the row below changes per hop.**

**Canonical reference implementation — Church of the Risen Christ, Toa Payoh (the most mature, 35/202 + 51 E2E + 51 built-artifact, `useScrollSpy` + `deepLinks`, `docs-contract` + `repo-hygiene`):** A reverent, editorial parish site for the first Catholic church in the new town, blessed 3 July 1971 by Archbishop Michel Olçomendy and Singapore's first fully air-conditioned church (Fr Pierre Abrial, $450k), at 91 Toa Payoh Central gathering the household in English, Mandarin, Tamil, Bahasa Indonesia, Tagalog — named for the Resurrection: He is risen (feast Easter Sunday). **For any new port (Parish X), replace the constants below with Parish X's `site.ts` — the architecture does not change.**

**Parish constants — Risen Christ example (canonical in `src/data/site.ts`; for Parish X, replace with Parish X's facts):**

| Fact | Value (Risen Christ example) | Source | Parish X template |
|---|---|---|---|
| Name | Church of the Risen Christ — `shortName` Risen Christ Toa Payoh — `chineseName` 耶稣复活堂 | `site.name / shortName / chineseName` | Parish X name/shortName/chineseName |
| Address | 91 Toa Payoh Central, Singapore 319193 | `site.address.full` (with `query` getter for maps) | Parish X address |
| Tagline / Vision | "Grateful, Faithful, and Sent." / "He is risen." | `site.tagline / site.vision` | Parish X tagline/vision |
| Patronal feast | The Risen Christ — **Easter Sunday** | `site.feast` | Parish X feast (St Joseph 1 May / Portiuncula 2 Aug / Easter) |
| Gates | Open for Mass, Adoration, and parish programmes | `site.hours.gates` | Parish X gates |
| Hours | 6 keys: `gates` (open for Mass/Adoration/programmes), `mainChurch`, `chapel` Adoration Room Mon 12–22 / Tue–Sat 7–22 / Sun 7–18, `reception` Mon–Fri 9–16 / Sat 9–12 / Sun 8–13, `parishOffice` Mon–Fri 9–16 / Sat 9–12 / Sun 8–13, `mediaCentre` Tue & Fri 12–16 / Sat 12–19 / Sun 8–13, `adorationRoom` (same as chapel) | `site.hours` | Parish X hours shape (St Mary: 7 keys with columbarium; St Joseph: 5 with bookshop) |
| Transport | MRT Toa Payoh NS19 — 6 min walk Exit A; buses 88, 157, 163 — 2 min from B52261 | `site.transport` | Parish X MRT/buses |
| Contacts | Parish priest +65 6255 7509, office +65 6253 2166, media centre +65 6356 5958, emails `crc.secretariat@catholic.org.sg` · `crc.admin@` / `crc.pastoral@` / `crc.youth@` / `dpo.crc@catholic.org.sg` | `site.contact` | Parish X phones/emails |
| Giving identity | UEN **T08CC4042G**, cheque payable **Church of the Risen Christ** | `site.uen / site.chequePayee` | Parish X UEN/chequePayee (St Mary T08CC4053H+HRSM; St Joseph T08CC4043C) |

**Lineage variants (examples — never mix outside the ledger):** St Joseph BT — 620 Upper BT, UEN T08CC4043C, 1 May St Joseph the Worker, cemetery; St Mary of the Angels — 5 Bukit Batok East Ave 2, UEN T08CC4053H+HRSM, 2 Aug Portiuncula, WOHA 2004, columbarium 7.30–21.30.

**Design thesis — "Reverent, not austere":** Warm parchment/maroon/gold on cream, generous whitespace, Fraunces display + Source Sans 3 body. Every page is a welcome from the parish's house of light — Toa Payoh Central's first air-con nave, Bukit Batok's WOHA folded planes, or Bukit Timah's Palladian hill — not a brochure. No purple gradients, no `Inter` defaults, no generic card-grid templates.

**Non-negotiable rules (apply to every hop):**

1. **Parish fidelity over pixel theft** — rephrase narrative, preserve Singapore facts exactly for the active hop (example: Risen 1969–2026 Toa Payoh — Ho Ping Centre, 1971 Olçomendy blessing, first air-con $450k, Velankanni, Simbang Gabi, 91 Toa Payoh Central, Mass 6.30a/6p + Sat 5.30p + Sun 5, UEN T08CC4042G). Never reintroduce predecessor narratives (Oklahoma/Guatemala, Palladian hill, WOHA Portiuncula) outside Appendices D/F — the active hop is the only parish in `src/data/*`.
...
> All versions pinned exact (no `^`) in `package.json` (`pnpm@11.0.0`, `engines: node>=20`). Re-pin on upgrade; `pnpm --frozen-lockfile` in CI verifies lockfile. `package.json` version is **3.0.0** for the Lineage Master (Risen Christ reference was 1.4.4, St Mary 1.2.0, St Joseph BT 1.3.0 — see Appendices D/F + Lineage Ledger).

**Environment:** No `.env`, no DB, no auth, no docker. `pnpm` is the supported manager (`--frozen-lockfile` in CI). `npm ci` fails on these exact pins (typescript-eslint 8.28.0 peer range predates TS 5.9) — use `npm ci --legacy-peer-deps` if npm is unavoidable. `skills/` is vendored reference content (ignored by tooling — eslint ignores, tsconfig excludes, vite watch ignores — do not import or lint it; history: pruned round-3, re-added `0be0fe8`). No `package-lock.json` (untracked — `pnpm` is the manager). `docs/ssh-key.txt` was accidentally tracked in `0be0fe8` and untracked in round-6 (`git rm --cached`, audit C1) — **the leaked key must be rotated by the repo owner**; history still contains it (`repo-hygiene` guards it). `src.orig/` is **forbidden in the git index for the entire lineage** (Rother → St Joseph → St Mary → Risen Christ) — lineage history lives in Appendices D/F + git history, not on disk; `repo-hygiene` test fails if any `src.orig` path re-enters the index (F-9: `.gitignore` alone does not untrack).

**Test harness — current reality (2026-08-31, Risen Christ reference — actual counts are machine-asserted by `docs-contract`; see `src/test/docs-contract.test.ts` + `repo-hygiene.test.ts` — do not hardcode prose):**
...
**Single source:** `src/index.css` `@theme` block. No `tailwind.config.*`. Tokens are the **unified 26+2 superset** — the Rother→St Joseph 24+2 baseline plus St Mary's `gold-700 #85601f` (4.72:1 AA) and Risen Christ's `terracotta-600 #8f4c30` (5.36:1 AA). Risen Christ intentionally refined to 25+2 (dropping `gold-700` as only `terracotta-600` was needed for its Devotion chip); the Lineage Master restores both AA steps for future ports. All imagery is parish-specific — the palette is lineage-wide, still warm parchment/maroon/gold on cream.
...
**Verification:** `grep --color shrine- src/index.css` → 26 colors + 2 shadows (28 theme entries = 26+2 superset: St Mary's `gold-700 #85601f` 4.72:1 + Risen's `terracotta-600 #8f4c30` 5.36:1); copy-paste `@theme` into this doc to prevent drift.
...
src/ (77 files — 41 source + 35 tests + 1 setup)
  ...
  index.css               # @theme (26 colors + 2 shadows = 28-theme superset: gold-700 + terracotta-600 AA) + @layer base/utilities (28 utilities: 27 + @media print) + 8 keyframes + themed scrollbar
  components/
...
Every hex matches `src/index.css` `@theme` byte-for-byte. **Fail the build if it drifts.** Lineage superset extends both AA text steps — `gold-700 #85601f` (4.72:1, St Mary) + `terracotta-600 #8f4c30` (5.36:1, Risen round-12 F-1 for Devotion chip); contract `src/components/wcag-contrast.test.tsx` computes ratios from this token layer; verification command `grep shrine- src/index.css` → 26 colors + 2 shadows (28 theme entries).
...
| `shrine-gold-700` | `#85601f` | `133,96,31` | `text-shrine-gold-700` | Gold AA text — 4.72:1 on parchment (St Mary, restored in superset) |
...
| ADR-3 | Tailwind v4 CSS-first `@theme` | Tokens co-located with CSS, no `tailwind.config.*` drift; `index.css` is the palette (26 colors + 2 shadows superset — Rother 24+2 + St Mary gold-700 #85601f + Risen terracotta-600 #8f4c30, both AA) | Extend `@theme` only, never arbitrary hex |
...
| ADR-6 | `src.orig/` reference policy (Lineage Master) | **Forbidden in the git index for the entire lineage** (Rother → St Joseph → St Mary → Risen); `eslint`/`vite watch` ignore entries are active guards; lineage history lives in Appendices D/F + git history, not on disk | `eslint` ignores `src.orig/`; `vite watch` ignores it; never import from it; do not re-add to `tsconfig.json` `include`; `repo-hygiene` test fails if any `src.orig` path re-enters |

---

## Appendix B — Live-Site Validation (Parameterized — Risen Christ Example Filled)

**Smoke script (manual or `agent-browser` — parameterize for Parish X; Risen Christ Toa Payoh example shown):**

```
# after pnpm build && pnpm preview (:4173) — Risen Christ Toa Payoh (91 Toa Payoh Central)
...
5. /worship#mass (direct) → lands on Mass schedule (three MassCards; card matching massDayKey(new Date()) carries data-today="true" + gold rule)
6. /worship#confession    → lands on Confession & Adoration
7. /worship#visit         → lands on Find Us (map embed + Toa Payoh NS19 + buses 88/157/163)
...
11. /news-events + /news-and-events → 6 events (Parish/Devotion/Formation/Archdiocese) — Risen: Velankanni 10–12 Sep + CEP + F.R.E.E. Acts (2 href)
12. /serve + /volunteer   → serveRoles 4 (title+summary) + devotions 6 (title+when+where — Adoration, Intercessory 2nd&4th Thu, Velankanni Sep, Simbang Gabi Dec, Bahasa 1st Fri, Tamil&Tagalog 2nd/4th Sun)
...
14. /faq                  → 6 Q&As via Accordion (grid-rows 0fr↔1fr + inert)
...
*Template: replace `{{parish}} priests {{N}} {{roles}}, UEN {{uen}}, timeline {{years}}, hours {{keys}}, anchors {{#list}}` per `src/data/site.ts` + `src/data/content.ts`. The 17-step structure is lineage-invariant; only the values change.*
...
| Tokens (26 colors + 2 shadows = 28-theme superset) + utilities (28 = 27 + `@media print`) + 8 keyframes | `src/index.css` (`--font-sans` alias `--font-body`; utilities incl. `gold-rule`/`gold-rule-left`/`hero-ken-burns`/`rise-in`+`rise-in-d1..d4`/`menu-in`/`drawer-in`/`drawer-item-in`/`page-in`/`dot-pulse`/`card-lift`/`card-tint`/`link-underline`/`skip-link`/`mask-fade-b`/`img-zoom`/`bg-gold-bloom`/`divider-weave`+`thin`/`bg-grain`+`adobe-texture`; motion kill `0.01ms` + `@media print` reveal) |
