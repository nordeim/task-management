...
> **💡 Zero-dependency fallback:** If Noto fonts are unavailable, ReportLab's built-in `UnicodeCIDFont('STSong-Light')` provides basic CJK support without external font files:
...
This uses Adobe's built-in CID font (installed with ReportLab) — zero external dependencies. Glyph coverage is limited compared to Noto Serif SC; prefer Noto when possible.
