...
|---|---|
| [`../pdf/`](../pdf/SKILL.md) | Canonical Z.AI PDF skill — four production lines (Report via ReportLab, Creative via JSON Blueprint + Playwright, Academic via Tectonic/LaTeX, Process for manipulate-existing). Use when you need the JSON Blueprint creative pipeline, `pdf_qa.py` 12-rule automated QA, or the cover system with 5 templates + Z-index layers. |
| [`../minimax-pdf/`](../minimax-pdf/SKILL.md) | MiniMax token-based design-system PDF with 14 document types (report, proposal, resume, portfolio, academic, general, minimal, stripe, diagonal, frame, editorial, magazine, darkroom, terminal, poster) each with a distinct cover pattern. Use when visual identity matters most and you want a pre-derifined doc-type → cover-pattern mapping. |
...
### Distinctive features of this skill (kimi-pdf)

- **HTML+Paged.js as the DEFAULT route** (LaTeX only when explicitly requested) — opposite default from the canonical `pdf` skill, which routes academic/math content to LaTeX automatically. Use kimi-pdf when you want HTML-first pagination with KaTeX/Mermaid; use canonical `pdf` when you want ReportLab structured documents or the Creative Blueprint pipeline.
- **11 cover style options** (see `routes/html.md`) — more granular than minimax-pdf's 14 doc-types (which bundle cover + body together) but less than canonical `pdf`'s 5-template × variant system.
- **Search-before-write enforcement** — the SKILL.md explicitly prohibits fabricating statistics, policies, research, or time-sensitive facts. If content involves ANY verifiable claim, you MUST search first. This is a hard constraint not emphasized in other pdf skills.
- **GB/T 7714 + APA citation standards** — Chinese refs use GB/T 7714 with `[J][M][D]` identifiers; English refs use APA. Mixed-language documents use each standard per-language. The canonical `pdf` skill mentions GB/T 7714 and APA but doesn't enforce the per-language split.
- **Clickable cross-references** — `<a href="#fig-1-1">Figure 1-1</a>` style internal links with `id` at container top. Required for academic papers.
- **Three-line tables** (三线表) — academic table style with top/middle/bottom rules only, no vertical lines. Native support in the HTML route.
- **Unified CLI** (`pdf.sh check | fix | html | latex`) — single entry point for all operations with consistent exit codes (0=success, 1=usage, 2=deps missing, 3=runtime). Simpler than the canonical `pdf` skill's `pdf.py` with 30+ subcommands.

### Shared rules (apply across all PDF skills)

- **Output language matches user's query language** — Chinese query → Chinese PDF; English query → English PDF; explicit user specification always wins.
- **Word/page count constraints** — strictly follow user-specified limits (±20% tolerance per canonical skill). Do not arbitrarily inflate content length.
- **Citations must be real** — fabricating references is prohibited. All citations must have correct author/institution names, accurate titles, verifiable year/journal/source.
- **Figures are block-level** — figures, diagrams, charts MUST be independent block elements at full width. NEVER float/wrap figures alongside body text.
- **Vector rendering iron rule** — final PDF MUST be generated via `page.pdf()` (Playwright) or ReportLab/LaTeX native output — NEVER via screenshot-to-PDF.
- **Full-bleed rule** — `@page { margin: 0 }` and `html, body { margin: 0; padding: 0 }` for any HTML→PDF. White side margins = broken layout.
- **Output cleanliness** — NEVER include version numbers, "DRAFT"/"CONFIDENTIAL" stamps, "Generated by AI", or any watermark the user didn't ask for.
