...
|---|---|
| [`../docx/`](../docx/SKILL.md) | Canonical Z.AI docx skill — JavaScript / `docx` (npm) pipeline with 7 cover recipes (R1–R7), scene router (academic/report/contract/resume/exam/official/copywriting), and `postcheck.py` 14-rule validator. Use when you want the JS-based pipeline instead of C#/.NET, or when you need the scene-specific routing system. |
| [`../minimax-docx/`](../minimax-docx/SKILL.md) | MiniMax C#/.NET OpenXML SDK pipeline with 13 aesthetic recipes (ModernCorporate, AcademicThesis, IEEE, ACM, APA 7th, MLA 9th, Chicago, Springer LNCS, Nature, HBR) and XSD validation gate-check. Use when you need authoritative style-guide compliance (IEEE/ACM/APA/MLA) rather than Kimi's Morandi/ink-wash design aesthetic. |
| [`../docx-generation/`](../docx-generation/SKILL.md) | Bidirectional Markdown ↔ DOCX conversion using the same Z.AI design-system rules as the canonical `docx` skill. Use when the source is Markdown and you want programmatic conversion with the 7-recipe cover system. |
| [`../pandoc-docx-template/`](../pandoc-docx-template/SKILL.md) | Pandoc-based DOCX with bundled Chinese reference templates (numbered/unnumbered headings, SCI paper layouts) and Lua filters. Use when you want Pandoc's reference-doc approach instead of programmatic generation. |
...
### Distinctive features of this skill (kimi-docx)

- **C# OpenXML SDK for creation** + **Python+lxml for editing** — strict separation, never mix. The canonical `docx` skill uses JS/docx-js for both; minimax-docx uses C# for both.
- **Native Word charts** (pie, bar, line) via `NumberLiteral` + `DataPoint` — editable in Word, small file size. Other skills use matplotlib PNG embeds.
- **Morandi + ink-wash background generators** (`generate_backgrounds.py`, `generate_inkwash_backgrounds.py`) — distinctive design aesthetic not found in other docx skills.
- **Pre-compiled OpenXML validator binary** (`validator/Validator`) — mandatory structural validation before delivery. Complements `postcheck.py` (canonical) and XSD gate-check (minimax).
- **Track Changes + Comments Python API** (`docx_lib.editing`) — `add_comment`, `insert_text`, `propose_deletion`, `enable_track_changes` with full revision mark handling. The most complete track-changes tooling of the three docx variants.
- **Fixed path conventions** (`/app/.kimi/skills/kimi-docx/`, `/tmp/docx-work/`, `/mnt/okcomputer/output/`) — designed for the Kimi sandbox environment. Adapt paths if running elsewhere.

### Shared rules (apply across all DOCX skills)

- **Language consistency**: Document language MUST match user's conversation language (including filename, body text, headings, headers, TOC hints, chart labels).
- **Headers/footers required by default**: Most documents MUST include headers (document title / chapter name) and footers (page numbers). Use `TitlePage` setting to hide on cover.
- **Cover/back cover separation**: Cover and back cover belong in their own sections with `TitlePage` enabled. Body content starts in a new section.
- **Element ordering**: OpenXML is strict about element order (`pPr` → runs in `w:p`; `rPr` → `t`/`br`/`tab` in `w:r`; `tblPr` → `tblGrid` → `tr` in `w:tbl`). Always run a structural validator after generation.
- **Track Changes structure**: `<w:ins>` and `<w:del>` WRAP `<w:r>` (not inside it). Deletions use `<w:delText>`, never `<w:t>`. Both must have `w:id` attribute.
- **Footnote/endnote separator trap**: `FootnotesPart`/`EndnotesPart` must include `Id=-1` (Separator) and `Id=0` (ContinuationSeparator) before any user notes. Missing these → Word fails to render.
