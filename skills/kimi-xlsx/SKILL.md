...
|---|---|
| [`../xlsx/`](../xlsx/SKILL.md) | Canonical Z.AI xlsx skill — openpyxl + pandas workbench with scene router (create/edit/analyze/convert/finance/finance_lite/advanced/vba), complexity gate (LITE vs FULL), `templates/base.py` design-token single source of truth, and `quality/pipeline.md` integrity workflow. Use when you want the scene-driven router, VBA macro support, or the design-token system. |
| [`../minimax-xlsx/`](../minimax-xlsx/SKILL.md) | MiniMax XML direct-edit pipeline — unpack → edit XML → repack, never openpyxl round-trip (which corrupts VBA/pivots/sparklines). Use when editing existing xlsx with zero format loss, or when you need the financial color standard (blue inputs / black formulas / green cross-sheet refs). |
...
| [`../charts/`](../charts/SKILL.md) | Standalone chart/diagram generation when the spreadsheet is just a chart carrier. |

### Distinctive features of this skill (kimi-xlsx)

- **KimiXlsx CLI tool** (6 commands) — the most comprehensive validation tooling of the three xlsx variants:
  - `recheck` — detects formula errors (#VALUE!, #DIV/0!, #REF!, #NAME?, #NULL!, #NUM!, #N/A), zero-value cells, and **implicit array formulas** that work in LibreOffice but fail in MS Excel (e.g., `MATCH(TRUE(), range>0, 0)` requires CSE). Suggests `SUMPRODUCT` alternatives.
  - `reference-check` — detects out-of-range references, header-row references, insufficient aggregate ranges, and inconsistent formula patterns ("isolated" formulas deviating from the column's predominant pattern).
  - `inspect` — structural analysis → JSON (sheets, tables, headers, data ranges).
  - `pivot` — PivotTable creation with optional chart via pure OpenXML SDK (the ONLY supported PivotTable method across all three xlsx skills; canonical `xlsx` uses openpyxl which has limited pivot support; minimax-xlsx skips pivots entirely).
  - `chart-verify` — verifies all charts have actual data content; exit code 1 = empty/broken charts that MUST be fixed.
  - `validate` — OpenXML structure validation (Office 2013 standard); checks PivotTable/Chart integrity, incompatible functions (FILTER/UNIQUE/XLOOKUP not supported in Excel 2019-), `.rels` path format. Files that fail CANNOT be opened by MS Excel.
- **Per-sheet validation workflow** — validate EACH sheet immediately after creation (recheck + reference-check → fix → next sheet), NOT after all sheets are done. This catches errors early when they're cheap to fix.
- **PivotTable + auto-chart** — `pivot` command automatically creates a bar/line/pie chart alongside the PivotTable. Two style themes: `monochrome` (default) and `finance`.
- **Cover pages with key metrics summary** — distinctive feature for executive-facing spreadsheets; not in canonical or minimax variants.
- **Currency formatting for financial data** — mandatory for tables involving financial/fiscal data (currency symbol prefix). Enforced as a hard rule.
- **Source citation tracking** — ALL external data (datasource, web_search, API calls) MUST have source citations. Two separate columns (`Source Name` | `Source URL`) or a dedicated "Sources" sheet. FORBIDDEN: delivering Excel with fetched data but no source references.

### Shared rules (apply across all XLSX skills)

- **Live formula guarantee** — every derived value SHOULD be an Excel formula so the spreadsheet stays dynamic. Exception: when the output will be verified by Python (not opened in Excel), write computed values because openpyxl can't evaluate formulas.
- **Zero error tolerance** — all divisions wrapped with `IFERROR` or `IF(denom=0,...)`. Absolute references (`$C$42`) for shared denominators.
- **Compatibility first** — no dynamic array functions (`FILTER`, `UNIQUE`, `XLOOKUP`, `SORT`, `SORTBY`, `XMATCH`, `SEQUENCE`, `LET`, `LAMBDA`, `RANDARRAY`). These break in Excel 2019 and earlier, Google Sheets, WPS, and LibreOffice. kimi-xlsx's `validate` command enforces this; canonical `xlsx` lists it as a rule; minimax-xlsx inherits it.
- **Preserve & match** — when editing existing files, study and exactly match the existing format, style, conventions. Existing patterns always override defaults. Text starting with `=` must be prefixed with `'`.
- **Language mirror** — output language (sheet names, headers, labels) matches user's input language.
- **Workbook metadata** — set creator to the appropriate value (`Z.ai` for canonical, or as appropriate for variant skills).
